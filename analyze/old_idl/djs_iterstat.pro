...
; REVISION HISTORY:
;
;       Mon Apr 21 17:12:19 2003, Erik Rosolowsky <eros@cosmic>
;		Behold, the might of the NAN keyword and NANs passed
;		for moments that don't exist!!
;16-Jun-1999 Written by David Schlegel, Princeton 11-Sep-2000 Speeded
;up by Hogg and Eisenstein 18-Sep-2000 Note change in MASK values to
;=1 for good (unrejected) points.  -
;------------------------------------------------------------------------------
pro djs_iterstat, image_in, sigrej=sigrej, maxiter=maxiter, $
 mean=fmean, median=fmedian, sigma=fsig, mask=mask, nan = nan, npoints = ngood
  ...
  image = image_in
   ...
   if keyword_set(nan) then begin
     null = where(image eq image, ngood)
     if ngood gt 0 then image = image[null]
   endif
